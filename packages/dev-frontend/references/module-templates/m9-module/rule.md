...
本规则用于审查和修正基于 `m9-module` 复制出来的业务模块。该模板使用 Jotai 作为模块级共享状态方案，并通过 `createModule(..., atom: moduleAtom)` 将状态源暴露给页面或其它业务模块。
...
`components/` 是按需目录；如果存在本地组件，必须有 `components/index.ts` 统一导出。
...
- `moduleAtom` 的类型来源必须是 `ScopeState`。
- 如果模块不需要跨组件共享状态，可以删除 `moduleAtom`，但必须同步删除入口的 `atom` 参数和 `useData` 中的 `useAtomState` 用法。

### type.ts

- 类型命名使用首字母大写驼峰。
- 保持类型链完整：`Props -> DataParams -> CtrlParams -> WatcherParams -> LayoutProps`。
- `Props` 默认继承 `ModuleProps<ModuleActions, ModuleDepAtoms>`。
- 对外回调用 `ModuleActions`，命令式方法用 `ModuleRef`。
- 模块本地数据写在 `DataState`。
- 跨组件共享状态写在 `ScopeState`，并由 `constant.ts` 中的 `moduleAtom` 初始化。
- 外部注入 atom 写在 `ModuleDepAtoms`，默认字段为 `scopeAtom?: ModuleAtom<ScopeState & { [key: string]: any }>`。
...
```ts
const [scopeState, setScopeState] = useAtomState(moduleAtom, MODULE_NAME);
```

- 如果当前模块消费外部注入状态，使用：

```ts
const scopeAtom = p.atoms?.scopeAtom ?? OwnerModule.atom;
const [scopeState, setScopeState] = useAtomState(scopeAtom, MODULE_NAME);
```
...
- 布局层不直接 import service，不直接读写 atom，不持有业务状态。
- 样式文件统一命名为 `style.module.less`。
- 本地组件保持展示职责；需要业务动作时通过 props 接收，不向上 import hook 或 service。

## 状态解耦约定

- `moduleAtom` 是该模块默认共享状态源。
- `createModule` 返回的组件通过 `.atom` 暴露该状态源。
- 其它模块如需复用该业务流状态，应优先由页面或编排层通过 `atoms` 注入，避免业务模块之间硬编码耦合。
- 允许兜底读取拥有者模块的 `.atom`，但 review 时应优先建议页面层显式注入。
