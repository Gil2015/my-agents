...
适用于 `packages/dev-frontend` 全流程技能包的共享约束。本文档只定义跨 step 都成立的工作区契约、文档维护规则和阶段边界；具体代码结构与示例以当前 mission 指定的业务模块模板为准。

## 1. 权威来源

- mission 工作区、文档产物和阶段职责边界，以本文件和 `../../README.md` 为准
- 前端模块结构、导出方式和占位文件，以 `config.json.moduleTemplate` 定位到的模板目录及其 `template.json` 为准
- 如果规则文字与当前业务模块模板冲突，应以当前模板为准完成当前任务，并尽快回写修正文档或模板元信息

## 2. Mission 与模块命名契约

- mission 工作区路径统一写作 `.ai/missions/{missionId}/...`
- 真实业务模块路径统一写作 `{projectRoot}/{moduleRoot}/{module.name}`；最终路径以当前业务模块模板 `targetPath` 的渲染结果为准
...
- 内置业务模块模板统一维护在 `../module-templates/{templateId}/`
- 目标项目可在 `.ai/dev-frontend/references/module-templates/{templateId}/` 维护项目内业务模块模板；项目内模板优先于工具内置模板
- step 私有指南、补充规则和分析方法继续放在各自的 `skills/*/references/` 下
- 项目级设计文档默认写入目标项目的 `.ai/docs/design-context.md`
- 项目级组件清单默认写入目标项目的 `.ai/docs/component-catalog.md`

## 4. 按需生成原则

- 初始化 mission 时只保证 `.ai/missions/{missionId}/config.json` 存在
- 项目级 `.ai/docs/` 不是固定产物，只有需要沉淀共享设计上下文或组件清单时才创建
- `reqDocs/` 由 `step1-req-collect` 首次创建
- `apiDoc/` 由 `step3-api-integrate` 首次创建
- `bugDocs/` 由 `step4-moduletest` 或 `step5-bug-fix` 首次创建
- `docs/` 由 `design-context-build` 首次创建
...
- `moduleTemplate.id`：当前 mission 使用的业务模块模板 ID，初始化默认写入 `m9-module`，开发人员可按项目约定修改
- `moduleTemplate.root`：显式业务模块模板路径；非空时优先使用该路径，不再自动回退
- `module.name`：业务模块目录名
...
- 在代码相关步骤中，必须先按 `moduleTemplate.root/id` 定位模板目录，并读取模板目录下的 `template.json`
- 如果 `config.json` 与文档顶部 `模块名` 冲突，应返回 `NEEDS_CONTEXT`，不要擅自选边
- 如果 `moduleTemplate.root` 或 `moduleTemplate.id` 无法解析到合法模板，应返回 `BLOCKED`，不要静默使用默认模板
...
- `design-context-build`：负责沉淀项目级设计上下文文档与组件清单，供 `step2-ui-dev` 复用
...
| 组件清单模板 | `../doc-templates/component-catalog-template.md` |
...
| 内置业务模块模板 | `../module-templates/{templateId}/` |
