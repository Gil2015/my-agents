...
根据 API 设计和数据库设计文档，在 NestJS 项目中按标准模块模板开发业务模块。输出必须能直接运行并通过基础测试，而不是只交付一个"先能编译"的骨架。
...
觉得"直接在 Controller 里写完所有逻辑"？停下来。混乱的分层会让后续每个阶段都慢下来。
...
如果模块分层、DTO 校验或 Entity 映射不符合模板，当前实现视为未完成；在继续缺陷修复或最终测试前，必须先回退并补齐结构。

## 第 1 步：读取上下文

读取以下信息：

- `.ai/missions/{missionId}/config.json`，确认 `backend.moduleName`、`backend.orm`、`backend.moduleRoot`
...
- `find "src/modules/{module-name}" -maxdepth 3 -type f | sort`
...
- `rg -n "__MODULE_NAME__|__TABLE_NAME__|__MODULE_CODE__" "src/modules/{module-name}"`
- `rg -n "Injectable|Controller|Entity|IsString|IsInt" "src/modules/{module-name}"`
- `find "src/modules/{module-name}" -maxdepth 3 -type f | sort`
...
| "先不注册 Module，跑测试就行"     | 不注册的模块在运行时不存在                   |
...
- 你创建了扁平模块，没有 dto/、entities/、vo/ 分层
- 你保留了模板占位符
