...
这个阶段只负责收集问题、补足证据并维护 `bugDocs/bug.md`，不负责修代码。交付物是一份可追溯、可排序、可继续交给 `bug-fix` 的缺陷清单。
...
如果 `.ai/missions/{missionId}/bugDocs/bug.md` 不存在、没有覆盖本轮问题来源、缺少证据，或顶部进度摘要失真，本轮问题收集视为未完成；`bug-fix` 不应启动。

## 执行流程

流程固定为 `LOAD → COLLECT → AUDIT → DRAFT → REVIEW → UPDATE`。

### 第 1 步：LOAD - 读取问题范围与来源

优先读取以下信息：

- `.ai/missions/{missionId}/config.json`：读取 `projectRoot`、`bugDocSources`、`backend.moduleName`、`backend.moduleRoot`
...
- `{projectRoot}/{backend.moduleRoot}/{module-name}/`：实际代码；若 `backend.moduleRoot` 缺失，按 `src/modules` 兜底
