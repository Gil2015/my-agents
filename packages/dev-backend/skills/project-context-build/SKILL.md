...
从 NestJS 项目代码、开发者补充信息和配置文件中，提炼出 AI 可消费的 `backend-context.md`。目标不是"总结框架文档"，而是沉淀一份全项目共用的后端架构基线，让后续 `api-design`、`db-design`、`module-dev` 准确复用项目既有的技术选型和约定，而不是临场猜测。
...
觉得"架构一看 `app.module.ts` 就知道了"？停下来。AI 不会长期记住这些隐性约束，必须写成文档。
...
没有证据支持的架构结论，只能标记为 `inferred` 或 `open_question`；绝不能伪装成项目既有规范。
...
- `projectRoot`：当前项目根目录
- 可选 `.ai/missions/{missionId}/config.json`：辅助获取 `projectRoot`
...
- `find "{projectRoot}/src" -name "*.module.ts" -maxdepth 4 | sort`
- `rg -n "TypeOrmModule|PrismaModule|SequelizeModule" "{projectRoot}/src"`
- `rg -n "APP_GUARD|APP_INTERCEPTOR|APP_FILTER|APP_PIPE" "{projectRoot}/src"`
- `rg -n "ConfigModule|ConfigService" "{projectRoot}/src"`
- `rg -n "JwtModule|PassportModule|AuthGuard" "{projectRoot}/src"`
...
| 借口                               | 现实                                         |
| ---------------------------------- | -------------------------------------------- |
| "看 package.json 就知道用什么 ORM" | 知道用什么和知道怎么用是两件事               |
...
- **可选触发者：** 开发者手动调用
- **非阻塞说明：** 缺少 `backend-context.md` 不阻塞后续 step，但各 step 必须显式说明架构上下文缺口
